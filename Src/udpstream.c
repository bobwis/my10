...
//		vTaskDelay(1); //some delay!
			...
			if ((sigsend) && (gpslocked)) { // only send if adc threshold was exceeded and GPS is locked
				...
				((uint8_t *) (pd->payload))[0] = statuspkt.udpcount & 0xff;
				((uint8_t *) (pd->payload))[1] = (statuspkt.udpcount & 0xff00) >> 8;
				((uint8_t *) (pd->payload))[2] = (statuspkt.udpcount & 0xff0000) >> 16;
				...
				if (jabber == 0) {		// don't actually send it if jabbering
					err = sendudp(pcb, pd, &udpdestip, UDP_PORT_NO);
