...
void SystemClock_Config(void)
{
	...
	PeriphClkInitStruct.PeriphClockSelection = RCC_PERIPHCLK_USART2 | RCC_PERIPHCLK_USART3
			| RCC_PERIPHCLK_USART6 | RCC_PERIPHCLK_UART4 | RCC_PERIPHCLK_UART5 | RCC_PERIPHCLK_UART7
			| RCC_PERIPHCLK_I2C1 | RCC_PERIPHCLK_I2C2 | RCC_PERIPHCLK_I2C4 | RCC_PERIPHCLK_CLK48;
...
static void MX_GPIO_Init(void)
{
	...
	/*Configure GPIO pin Output Level */
	HAL_GPIO_WritePin(GPIOD, LED_D1_Pin | LED_D2_Pin | LED_D3_Pin | LED_D4_Pin | LED_D5_Pin,
			GPIO_PIN_SET);
	...
	/*Configure GPIO pins : PE3 PE7 PE8 PE11
	 PE12 PE13 PE1 */
	GPIO_InitStruct.Pin = GPIO_PIN_3 | GPIO_PIN_7 | GPIO_PIN_8 | GPIO_PIN_11 | GPIO_PIN_12
	...
	/*Configure GPIO pins : PG0 PG1 PG3 PG4
	 PG5 PG10 PG12 PG14
	 PG15 */
	GPIO_InitStruct.Pin = GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_3 | GPIO_PIN_4 | GPIO_PIN_5 | GPIO_PIN_10
...
void calcagc()
{
	...
	if (samples > GAININT)	{
		...
		if (noiseacc < (3 * GAININT))	// too quiet
		{
			if (newgain < 7) {
...
///			HAL_TIM_Base_Start(&htim7);	// audio synth sampling interval timer
			...
			if (tenmstimer % 100 == 0) {		// every second
				...
				if (gpslocked)
					sprintf(gpsstr,"\"Locked: %d Sats<br>Lon: %d<br>Lat: %d\"",statuspkt.NavPvt.numSV,statuspkt.NavPvt.lon,statuspkt.NavPvt.lat);
				else
